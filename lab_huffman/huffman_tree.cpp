...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	 ...
	 */
/*	     vector<Frequency>::const_iterator iter;
        for(iter = frequencies.begin(); iter < frequencies.end(); iter++)
        {
            TreeNode * foo = new TreeNode(*iter);
        ...
        while(1)
        {
            ...
            TreeNode * bar = new TreeNode(first -> freq.getFrequency() + second -> freq.getFrequency() );
            bar -> left = first;
            bar -> right = second;
            mergeQueue.push(bar);
       
            if(mergeQueue.empty() || singleQueue.empty() )
                if(mergeQueue.size() == 1 || singleQueue.size() == 1)
                    break;
        }
...
*/
	...
	while((singleQueue.size()+mergeQueue.size())>1)	
	{
		...
		TreeNode * node = new TreeNode((first_node->freq).getFrequency()+(second_node->freq).getFrequency());
...
void HuffmanTree::decode( stringstream & ss, BinaryFileReader & bfile ) {
	...
	while( bfile.hasBits() ) {
		...
		/*bool hasBits = bfile.getNextBit();
            ...
            // if the node has no children it is a leaf and
            	else
            	{
                	ss << (current -> freq.getCharacter() );
                	current = root;
	...
	if(bfile.hasBits() )
        {
           ...
           // bit is one
            ...
            {
                foo = new TreeNode(0);
                return foo -> left = readTree(bfile);
                return foo -> right = readTree(bfile);
            }  
        }
        // no bits - no creation
        return NULL;*/
