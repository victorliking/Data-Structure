...
animation filler::fill( PNG & img, int x, int y,
	...
	while(!xcoords.isEmpty())
	{
		...
		if(curr_x<0||curr_y<0||curr_x>=img.width()||curr_y>=img.height())
		{
			continue;		
		}
		RGBAPixel * curr = img(curr_x,curr_y);
		int red_value = (int)(curr->red - b.red);
		int green_value = (int)(curr->green - b.green);
		int blue_value = (int)(curr->blue - b.blue);
		int checker = red_value*red_value + green_value*green_value+blue_value*blue_value;
		if(curr!=NULL&&process[curr_x][curr_y]==false&&checker<=tolerance)
		{
